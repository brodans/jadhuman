...
// ─── Deteksi mobile (module-level agar bisa dipakai di startCamera) ───────────
const IS_MOBILE = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
  typeof navigator !== 'undefined' ? navigator.userAgent : ''
);

// ─── EXIF helpers (module-level, reuse di processFile) ───────────────────────
/**
 * Ekstrak segmen APP1 (EXIF) dari buffer JPEG asli.
 * Return Uint8Array berisi marker + length + data, atau null jika tidak ada.
 */
function extractExifSegment(buffer: ArrayBuffer): Uint8Array | null {
  ...
  while (offset + 4 <= view.byteLength) {
    ...
    if (marker === 0xFFE1) {
      // APP1 = EXIF; segLen termasuk 2 byte field panjang itu sendiri
      const segLen = view.getUint16(offset + 2);
      return new Uint8Array(buffer, offset, segLen + 2); // +2 utk marker
    }
    if ((marker & 0xFF00) !== 0xFF00) break; // marker tidak valid
    const segLen = view.getUint16(offset + 2);
    offset += 2 + segLen;
  }
  return null;
...
/**
 * Inject segmen EXIF ke dalam JPEG data URL hasil canvas.
 * Disisipkan tepat setelah SOI (0xFFD8), sebelum APP0/data lainnya.
 */
function injectExifIntoDataUrl(canvasDataUrl: string, exifSegment: Uint8Array): string {
  ...
  // SOI (2 byte) + EXIF segment + sisa byte (mulai offset 2)
...
export default function TabAbsen() {
  const { pegawai, config, developerMode, userRole, tabPermissions } = useAppContext();
  ...
  const processFile = (file: File) => {
    ...
    // Baca dua kali bersamaan: ArrayBuffer (untuk EXIF) + DataURL (untuk Image)
    ...
    arrayBufReader.onload = (abEvt) => {
      ...
      dataUrlReader.onload = (duEvt) => {
        ...
        img.onload = () => {
          ...
          // Cover crop — isi 240×320 tanpa distorsi, crop tengah
          const srcW  = img.naturalWidth  || img.width;
          const srcH  = img.naturalHeight || img.height;
          const scale = Math.max(TARGET_W / srcW, TARGET_H / srcH);
          const sw    = srcW * scale;
          const sh    = srcH * scale;
          ctx.drawImage(img, (TARGET_W - sw) / 2, (TARGET_H - sh) / 2, sw, sh);
          ...
          if (exifSegment) {
            ...
            catch (e) { console.warn('[TabAbsen] Gagal inject EXIF:', e); }
          }
  ...
  const startCamera = async () => {
    // Mobile: buka kamera native Android/iOS → dapat EXIF asli
    if (isMobile) {
      cameraInputRef.current?.click();
      return;
    }
    // Desktop: buka webcam via getUserMedia
    try {
  ...
  const capturePhoto = () => {
    ...
    // Cover crop dari live webcam feed
    ...
    const sw    = srcW * scale;
    const sh    = srcH * scale;
    ctx.drawImage(videoRef.current, (TARGET_W - sw) / 2, (TARGET_H - sh) / 2, sw, sh);

    canvas.toBlob((blob) => {
      if (blob) {
        processFile(new File([blob], 'webcam-capture.jpg', { type: 'image/jpeg' }));
        stopCamera();
      }
    }, 'image/jpeg', 0.92);
  };
