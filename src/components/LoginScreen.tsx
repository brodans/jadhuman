...
import {
  ...
  Sun, Moon, User, ShieldAlert, CreditCard, X
} from 'lucide-react';
...
export default function LoginScreen({ onLogin, isDarkMode, toggleDarkMode }: LoginScreenProps) {
  ...
  const pendingLoginRef = useRef<{ user: UserAccountSafe; permissions: TabPermissions } | null>(null);
  ...
  const handleAuthentication = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (authSnap.exists()) {
        ...
        if (authenticated) {
          const storedAdminUser = typeof data.adminUsername === 'string'
            ? data.adminUsername.toLowerCase().trim()
            : '';
          loginAsUser = createAdminUserAccount(storedAdminUser || uname || 'admin');
      ...
      if (authenticated && loginAsUser) {
        ...
        // Admin utama (role='admin' dari settings/auth) bypass payment
        const isMainAdmin = loginAsUser.role === 'admin';
        if (!isMainAdmin && loginAsUser.id) {
          try {
            let sub = await getSubscription(loginAsUser.id);
            if (sub) sub = await checkAndAutoExpire(sub);
            if (!isSubscriptionActive(sub)) {
              // Tampilkan modal payment — tahan onLogin() sampai lunas
              pendingLoginRef.current = { user: loginAsUser, permissions: loginPermissions };
              setPendingSubscription(sub);
              setShowPaymentModal(true);
              return; // jangan panggil onLogin() dulu
            }
          } catch (subErr) {
            console.warn('[LoginScreen] Gagal cek subscription, lanjutkan:', subErr);
          }
        }
  ...
  // ── Helper: load Midtrans Snap JS ────────────────────────────────────────
  const loadSnapJs = (): Promise<void> => new Promise((resolve, reject) => {
    ...
    s.onload = () => resolve();
    s.onerror = () => reject(new Error('Gagal load Midtrans Snap JS'));
    document.head.appendChild(s);
  });

  // ── Handler: buka Midtrans Snap ──────────────────────────────────────────
  const handleBayar = async () => {
    ...
    try {
      ...
      (window as any).snap.pay(data.snap_token, {
        onSuccess: async () => {
          // Webhook Midtrans mungkin belum tiba — coba re-check sampai 3x
        ...
        onPending: () => {
          setPaymentError('Pembayaran menunggu konfirmasi. Silakan tunggu atau coba lagi.');
        },
        onError: () => {
    ...
    <div className="min-h-screen relative flex flex-col justify-center items-center p-6 transition-colors duration-700 font-sans overflow-hidden bg-slate-100 dark:bg-slate-950">
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-sm p-6 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">Aktivasi Akun</h3>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">Diperlukan untuk mengakses sistem</p>
                </div>
              </div>
              <button
                onClick={() => { setShowPaymentModal(false); dispatch({ type: 'UPDATE_COOLDOWN', payload: null }); }}
            ...
            {/* Info */}
            <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 rounded-2xl space-y-2">
              <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">
                Akun <span className="font-mono">@{pendingLoginRef.current?.user.username}</span> belum aktif.
              </p>
              <p className="text-xs text-amber-700 dark:text-amber-400">
                Lakukan pembayaran sebesar{' '}
                <span className="font-bold text-amber-900 dark:text-amber-200">
                  {formatRp(pendingSubscription?.amount ?? 50000)}
                </span>
                {pendingSubscription?.billing_type === 'monthly' ? ' per bulan' : ''} untuk mengakses aplikasi.
              </p>
              {pendingSubscription?.status === 'expired' && (
                <p className="text-[11px] text-rose-600 dark:text-rose-400 font-semibold">
                  ⚠️ Masa aktif akun telah berakhir.
                </p>
              )}
            </div>
            ...
            {/* Error */}
            {paymentError && (
              <div className="p-3 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-700/40 rounded-xl text-xs text-rose-600 dark:text-rose-400 font-semibold">
                {paymentError}
              </div>
            )}
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 rounded-2xl text-sm transition-all"
              >
                {paymentLoading
                  ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /><span>Memproses...</span></>
                  : <><CreditCard className="w-4 h-4" /><span>Bayar Sekarang</span></>
                }
              </button>
            </div>
          </div>
