...
async function startServer() {
  ...
  app.post("/api/test-notification", async (req, res) => {
    ...
    try {
      ...
      const title     = '⏰ Pengingat Produktivitas [TEST]';
      const tag       = 'productivity-test';
      const targetUrl = '/input-aktivitas';
      const type      = 'productivity_reminder';

      const twoHrsAgo = new Date(now.getTime() - 2 * 60 * 60 * 1000);
      const formatted = twoHrsAgo.toLocaleString("id-ID", {
        day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit",
      });
      const body = `${displayName} telah bekerja 2 jam! (Mulai: ${formatted}). Segera akhiri sesi.`;
  ...
  app.post("/api/subscription-create", async (req, res) => {
    ...
    if (!serverKey) {
      return res.status(500).json({ error: "MIDTRANS_SERVER_KEY tidak ditemukan di environment variables" });
    }
    ...
    try {
      initAdmin();
      const db = getFirestore();
      const subRef = db.collection("jadhuman_subscriptions").doc(user_id);
      ...
      if (subSnap.exists) {
        ...
        amount = typeof subData.amount === "number" ? subData.amount : 50000;
        billingType = subData.billing_type || "monthly";
      }
          ...
          item_details: [
            {
              ...
              price: amount,
              quantity: 1,
              name: `Langganan Jadhuman${billingType === "monthly" ? " (Bulanan)" : ""}`,
            },
          ],
        }),
      });
  ...
  app.post("/api/subscription-notify", async (req, res) => {
    ...
    try {
      ...
      const match = order_id.match(/^(?:jadhuman-)?sub-(.+)-[a-z0-9]+$/i);
      ...
      initAdmin();
      const db = getFirestore();
      const subRef = db.collection("jadhuman_subscriptions").doc(userId);
      const now = new Date();
      ...
      if (isSuccess) {
        ...
        console.log(`[subscription-notify] ✅ User ${userId} subscription diaktifkan`);
      } else if (isFailure) {
        await subRef.set(
          {
            status: "unpaid",
